...
import { postActivenessFlag } from "../../features/posts/postActivenessFlagSlice.js";
...
const MyListings = () => {
  ...
  const { loading } = useSelector((state) => state.postActivenessFlag);
  ...
  const setpostActivenessFlag = (item) => {
    dispatch(
      postActivenessFlag({ postId: item.postId, myListingId: item._id, }))
      .unwrap()
      .then(() => {
        dispatch(fetchMyListings());
      })
    ...
    console.log("postId", item.postId, "myListingId", item._id);
  };
                ...
                <div className="p-5">
                  ...
                  <div className="flex gap-2">
                    ...
                    {item.status === "active" ? (
                      ...
                      <motion.button
                        onClick={() => setpostActivenessFlag(item)}
                        disabled={loading}
                        whileHover={{ scale: 1.05 }}
                        whileTap={{ scale: 0.95 }}
                        className="flex-1 flex items-center justify-center px-3 py-2 bg-gradient-to-r from-green-500 to-green-600 
                 dark:from-green-600 dark:to-green-700 text-white rounded-lg hover:from-green-600 hover:to-green-700 
                 transition-all text-sm font-medium shadow-md"
                      >
                        <CheckCircle className="h-3 w-3 mr-1" />
                        {loading ? "Updating..." : "I Found This"}
                      </motion.button>
                    )}
                  </div>
                </div>
